...
#define MESSAGE_SIZE 10 //10 + 1 + 2 .
#define PACKET_SIZE 14
#define RECEIVE_BUFFER_SIZE 262144 // power of 2
...
void set_packet(unsigned short* seq,int *buffer_index,struct message_packet*buffer,
  char* message,struct message_packet *packet){
    memset(packet, '\0', PACKET_SIZE);
    memset((*packet).message,'\0',MESSAGE_SIZE);
    (*packet).seq_no = *(seq); // set packet's seq_no
    (*packet).packet_type = 0; // 0 for message packets, 1 for ACK
    strncpy((*packet).message,message,MESSAGE_SIZE);
    memcpy(buffer+ (*buffer_index),packet,PACKET_SIZE);
    return;
}
...
void *send_packets(void *vargp){
  ...
  if( newlineTotal >= 2 ){
      ...
      for(int j = 0 ; j < 20; j++){
        if ((numbytes = sendto(sockfd,&packet, PACKET_SIZE, 0,
  ...
  for(int j = 0; j < length_of_line; j+=MESSAGE_SIZE){
    ...
    if( s_window.next_seq_sum - s_window.send_base < s_window.window_size ){
      ...
      if ((numbytes = sendto(sockfd,&buffer[buffer_index], PACKET_SIZE, 0,
                             p->ai_addr, p->ai_addrlen)) != -1)
      {
              //printf("Packet SENT!\n" );
    ...
    /*printf (" pkg message: %.14s . seqNo: %d , type: %d \n",
...
void *receive_packets(void * vargp){
  ...
  if ((numbytes = recvfrom(sockfd2, &packet_received, sizeof(struct message_packet), 0,
                           (struct sockaddr *)&their_addr, &addr_len)) != -1)
  {
      if( packet_received.packet_type == 0 ){ // normal packet (NOT ACK)
        ...
        inet_ntop(their_addr.ss_family,
                    ...
                    s, sizeof s));*/
        /*printf("server: packet contains: %.14s,seqno: %d , packet_type: %d message_size: %ld\n",
          ...
          if( r_window.rcv_base == packet_received.seq_no ){ // in-order packet received
              ...
              if((numbytes = sendto(sockfd,&packet, sizeof(struct message_packet), 0,
                                       p->ai_addr, p->ai_addrlen)) != -1){
      ...
      else{ // received_packet_type == 2, exit
...
void setTimeout()
{
    ...
    if( previous_send_base == s_window.send_base ){
      ...
      for(int i = s_window.send_base; i < s_window.next_seq_sum &&
        ...
        if ((numbytes = sendto(sockfd,&buffer[i], PACKET_SIZE, 0,
                                p->ai_addr, p->ai_addrlen)) != -1)
        {
    ...
    /*if( count == 8 ){
      ...
      if( previous_send_base == s_window.send_base &&
        ...
        previous_rcv_base == buffer_index_receive){
          printf("senderbase: %d, receivebase: %d, bufferindex: %d buffer_receiveindex: %d\n",
         s_window.send_base,r_window.rcv_base,buffer_index,buffer_index_receive);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        fprintf(stderr, "usage: ./client <172.24.0.10> <server_port>\n");
        exit(1);
    }
    ...
    if ((rv = getaddrinfo("172.24.0.10", argv[2], &hints, &servinfo)) != 0)
    {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
    }
    if ((rv = getaddrinfo(NULL, argv[2], &hints2, &servinfo2)) != 0)
    {
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
        return 1;
    }
